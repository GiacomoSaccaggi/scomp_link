# -*- coding: utf-8 -*-
"""
scomp-link: The Astromech Arm for Your Python Projects.

All public classes are lazily imported via PEP 562 (__getattr__).
Only the logger is loaded eagerly. Each class loads its dependencies
on first access, keeping `import scomp_link` near-instant.
"""

from .utils.logger import set_verbosity

__version__ = "2.3.1"

__all__ = [
    "set_verbosity",
    "__version__",
    "ScompLinkPipeline",
    "Preprocessor",
    "FeatureEngineer",
    "DataQualityReport",
    "ModelFactory",
    "RegressorOptimizer",
    "ClassifierOptimizer",
    "AnomalyDetector",
    "TimeSeriesAnomalyDetector",
    "TimeSeriesForecaster",
    "OptunaOptimizer",
    "HalvingSearchOptimizer",
    "EarlyStoppingCV",
    "Validator",
    "FairnessMetrics",
    "ShapExplainer",
    "LimeExplainer",
    "DriftDetector",
    "ScompArtifact",
    # DSL
    "Chain",
    "CleanStep",
    "SelectStep",
    "ModelStep",
    "TrainStep",
    "SectionStep",
    "TitleStep",
    "TextStep",
    "GraphStep",
    "TableStep",
    "RawGraphStep",
    "SaveStep",
    "CodeStep",
    "DiffStep",
    "MermaidStep",
    "TerminalStep",
    # LLM
    "FineTuner",
    "TransformerBuilder",
    "ModelConverter",
    "LLMFactory",
    "TransformerConfig",
    "FineTuneConfig",
    "LLMFineTuneStep",
    "LLMConvertStep",
    "LLMSaveStep",
    "ModelMerger",
    "InferenceServer",
    "DatasetFormatter",
    "TextDeduplicator",
    "TextQualityMetrics",
    "NGramAnalyzer",
    "BLEUScore",
    "ROUGEScore",
    "TextFilter",
    "Conversation",
    "DedupResult",
    "LLMEvalStep",
    "LLMFormatStep",
    "LLMDedupStep",
    "LLMMergeStep",
]

_LAZY_IMPORTS = {
    # models
    "RegressorOptimizer": (".models.regressor_optimizer", "RegressorOptimizer"),
    "ClassifierOptimizer": (".models.classifier_optimizer", "ClassifierOptimizer"),
    "ModelFactory": (".models.model_factory", "ModelFactory"),
    "AnomalyDetector": (".models.anomaly_detector", "AnomalyDetector"),
    "TimeSeriesAnomalyDetector": (".models.ts_anomaly_detector", "TimeSeriesAnomalyDetector"),
    "TimeSeriesForecaster": (".models.forecaster", "TimeSeriesForecaster"),
    # tuning
    "OptunaOptimizer": (".models.advanced_tuning", "OptunaOptimizer"),
    "HalvingSearchOptimizer": (".models.advanced_tuning", "HalvingSearchOptimizer"),
    "EarlyStoppingCV": (".models.advanced_tuning", "EarlyStoppingCV"),
    # core
    "ScompLinkPipeline": (".core", "ScompLinkPipeline"),
    # preprocessing
    "Preprocessor": (".preprocessing.data_processor", "Preprocessor"),
    "FeatureEngineer": (".preprocessing.feature_engineer", "FeatureEngineer"),
    "DataQualityReport": (".preprocessing.data_quality", "DataQualityReport"),
    # validation
    "Validator": (".validation.model_validator", "Validator"),
    "FairnessMetrics": (".validation.fairness", "FairnessMetrics"),
    # explainability
    "ShapExplainer": (".explainability", "ShapExplainer"),
    "LimeExplainer": (".explainability", "LimeExplainer"),
    # monitoring
    "DriftDetector": (".monitoring", "DriftDetector"),
    # persistence
    "ScompArtifact": (".persistence", "ScompArtifact"),
    # DSL
    "Chain": (".pipeline_dsl", "Chain"),
    "CleanStep": (".pipeline_dsl", "CleanStep"),
    "SelectStep": (".pipeline_dsl", "SelectStep"),
    "ModelStep": (".pipeline_dsl", "ModelStep"),
    "TrainStep": (".pipeline_dsl", "TrainStep"),
    "SectionStep": (".pipeline_dsl", "SectionStep"),
    "TitleStep": (".pipeline_dsl", "TitleStep"),
    "TextStep": (".pipeline_dsl", "TextStep"),
    "GraphStep": (".pipeline_dsl", "GraphStep"),
    "TableStep": (".pipeline_dsl", "TableStep"),
    "RawGraphStep": (".pipeline_dsl", "RawGraphStep"),
    "HighchartsStep": (".pipeline_dsl", "HighchartsStep"),
    "SubtitleStep": (".pipeline_dsl", "SubtitleStep"),
    "SaveStep": (".pipeline_dsl", "SaveStep"),
    "LogStep": (".pipeline_dsl", "LogStep"),
    "CodeStep": (".pipeline_dsl", "CodeStep"),
    "DiffStep": (".pipeline_dsl", "DiffStep"),
    "MermaidStep": (".pipeline_dsl", "MermaidStep"),
    "TerminalStep": (".pipeline_dsl", "TerminalStep"),
    # LLM
    "FineTuner": (".llm.training.finetune", "FineTuner"),
    "TransformerBuilder": (".llm.training.scratch", "TransformerBuilder"),
    "ModelConverter": (".llm.serving.convert", "ModelConverter"),
    "LLMFactory": (".llm.core.factory", "LLMFactory"),
    "TransformerConfig": (".llm.core.configs", "TransformerConfig"),
    "FineTuneConfig": (".llm.core.configs", "FineTuneConfig"),
    "LLMFineTuneStep": (".llm.dsl", "LLMFineTuneStep"),
    "LLMConvertStep": (".llm.dsl", "LLMConvertStep"),
    "LLMSaveStep": (".llm.dsl", "LLMSaveStep"),
    "ModelMerger": (".llm.serving.merge", "ModelMerger"),
    "InferenceServer": (".llm.serving.serve", "InferenceServer"),
    "DatasetFormatter": (".llm.data.formatting", "DatasetFormatter"),
    "TextDeduplicator": (".llm.data.dedup", "TextDeduplicator"),
    "TextQualityMetrics": (".llm.evaluation.quality", "TextQualityMetrics"),
    "NGramAnalyzer": (".llm.evaluation.ngrams", "NGramAnalyzer"),
    "BLEUScore": (".llm.evaluation.bleu", "BLEUScore"),
    "ROUGEScore": (".llm.evaluation.rouge", "ROUGEScore"),
    "TextFilter": (".llm.data.dedup", "TextFilter"),
    "Conversation": (".llm.data.formatting", "Conversation"),
    "DedupResult": (".llm.data.dedup", "DedupResult"),
    "LLMEvalStep": (".llm.dsl", "LLMEvalStep"),
    "LLMFormatStep": (".llm.dsl", "LLMFormatStep"),
    "LLMDedupStep": (".llm.dsl", "LLMDedupStep"),
    "LLMMergeStep": (".llm.dsl", "LLMMergeStep"),
}


def __getattr__(name):
    if name in _LAZY_IMPORTS:
        module_path, attr_name = _LAZY_IMPORTS[name]
        import importlib

        module = importlib.import_module(module_path, __package__)
        obj = getattr(module, attr_name)
        globals()[name] = obj
        return obj
    raise AttributeError(f"module 'scomp_link' has no attribute {name!r}")


def __dir__():
    return list(_LAZY_IMPORTS.keys()) + ["set_verbosity", "__version__"]
