...
def barchart(
    categories: list, values: list, title: str = "", width: int = 800, height: int = 500, colors: list | None = None
) -> str:
...
def barchartmultiset(
    categories: list, groups: dict, title: str = "", width: int = 800, height: int = 500, colors: list | None = None
) -> str:
...
def piechart(
    labels: list, values: list, title: str = "", width: int = 600, height: int = 600, colors: list | None = None
) -> str:
...
def radarchart(
    categories: list, series: dict, title: str = "", width: int = 600, height: int = 600, colors: list | None = None
) -> str:
...
def voronoidiagram(
    ...
    for simplex in vor.ridge_vertices:
        if -1 not in simplex:
            ax.plot(
                [vor.vertices[simplex[0], 0], vor.vertices[simplex[1], 0]],
...
if __name__ == "__main__":
    ...
    # barchartmultiset
    svg = barchartmultiset(
        ["Jan", "Feb", "Mar"], {"Product A": [30, 45, 28], "Product B": [22, 38, 41]}, "Sales by Product"
    )
    report.add_rawgraphs_to_report(svg, "Multi-set Bar Chart")

    # barchartstacked
    svg = barchartstacked(
        ["2021", "2022", "2023"], {"Online": [100, 150, 200], "Retail": [80, 90, 110]}, "Revenue Channels"
    )
