...
def contour_plot(
    x: list, y: list, title: str = "", width: int = 800, height: int = 600, colors: list | None = None, levels: int = 10
) -> str:
...
def convex_hull(
    x: list, y: list, groups: list, title: str = "", width: int = 800, height: int = 600, colors: list | None = None
) -> str:
...
def parallelcoordinates(
    ...
    if class_column and class_column in data:
        ...
        legend_elements = [
            Line2D([0], [0], color=colors[i % len(colors)], label=c) for i, c in enumerate(unique_classes)
        ]
...
if __name__ == "__main__":
    ...
    # parallelcoordinates
    data = {
        "Speed": np.random.rand(30).tolist(),
        "Power": np.random.rand(30).tolist(),
        "Efficiency": np.random.rand(30).tolist(),
        "Cost": np.random.rand(30).tolist(),
        "Type": (["A"] * 15 + ["B"] * 15),
    }
