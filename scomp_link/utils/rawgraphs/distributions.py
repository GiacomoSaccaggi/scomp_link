...
def beeswarm(
    ...
    if groups is None:
        groups_arr = np.array([""] * len(data_arr))
    else:
        groups_arr = np.asarray(groups)
    unique_groups = list(dict.fromkeys(groups_arr))
...
if __name__ == "__main__":
    ...
    # beeswarm
    data = np.concatenate([np.random.normal(5, 1, 50), np.random.normal(8, 1.5, 50), np.random.normal(6, 0.8, 50)])
    ...
    # boxplot
    data_box = [
        np.random.normal(10, 2, 100).tolist(),
