...
def bumpchart(
    ranks: dict, periods: list, title: str = "", width: int = 800, height: int = 500, colors: list | None = None
) -> str:
...
def gantt_chart(tasks: list, title: str = "", width: int = 900, height: int = 500, colors: list | None = None) -> str:
    ...
    for i, task in enumerate(tasks):
        ...
        ax.broken_barh(
            [(task["start"], duration)], (y_pos - 0.35, 0.7), facecolors=c, edgecolors="white", linewidth=0.5
        )
...
def linechart(
    series: dict, x_values: list, title: str = "", width: int = 800, height: int = 500, colors: list | None = None
) -> str:
    """Multi-line chart."""
    colors = colors or COLORS
    fig, ax = plt.subplots(figsize=(width / 100, height / 100))
...
if __name__ == "__main__":
    ...
    # bumpchart
    svg = bumpchart(
        {"Apple": [1, 2, 1, 1], "Google": [2, 1, 3, 2], "Microsoft": [3, 3, 2, 3]},
        ["2020", "2021", "2022", "2023"],
        "Market Rank",
    )
    ...
    # gantt_chart
    tasks = [
        {"name": "Design", "start": 0, "end": 3, "group": "Phase 1"},
    ...
    svg = horizongraph(
        {
            "Temperature": (np.sin(np.linspace(0, 4 * np.pi, 50)) * 10).tolist(),
            "Pressure": (np.cos(np.linspace(0, 3 * np.pi, 50)) * 5).tolist(),
        },
        x,
        "Sensor Data",
    )
    ...
    # linechart
    svg = linechart(
        {"Revenue": [10, 15, 13, 18, 22, 25], "Costs": [8, 9, 11, 12, 14, 15]},
    ...
    # slopechart
    svg = slopechart(
        {"Product A": [45, 62], "Product B": [70, 55], "Product C": [30, 48]}, ["2022", "2023"], "Year-over-Year Change"
    )
    ...
    svg = streamgraph(
        {
            "Rock": np.random.randint(5, 20, 20).tolist(),
            "Pop": np.random.randint(3, 15, 20).tolist(),
            "Jazz": np.random.randint(2, 10, 20).tolist(),
        },
        x,
        "Music Trends",
    )
