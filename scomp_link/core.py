...
class ScompLinkPipeline:
    ...
    def __rshift__(self, other: Any) -> Any:
        """Enable pipeline >> step syntax. Binds this pipeline instance to the resulting Chain."""
        from .pipeline_dsl import Chain, MLStep, _BoundPipelineStep

        if isinstance(other, (MLStep, Chain)):
            bound = _BoundPipelineStep(self)
            if isinstance(other, Chain):
                return Chain([bound, *other._steps])
            return Chain([bound, other])
        raise TypeError(f"Cannot chain ScompLinkPipeline with {type(other)}")
    ...
    def import_and_clean_data(self, df: pd.DataFrame):
        ...
        array_cols = [
            c for c in df.columns if df[c].dtype == object and len(df) > 0 and isinstance(df[c].iloc[0], np.ndarray)
        ]
        if array_cols:
    ...
    def select_variables(self, target_col: str, feature_cols: Optional[List[str]] = None):
        ...
        assert self.df is not None, "Call import_and_clean_data() first"
    ...
    def run_pipeline(
        ...
        if task_type == "clustering":
            ...
            self.results = {
                ...
                "metrics": {metric_name: metric_value, "silhouette_score": silhouette},
            }
            return self.results
        ...
        if task_type == "text" and text_col:
            ...
            try:
                if use_contrastive:
                    ...
                    classifier = ContrastiveTextClassifier(model_name=text_model, embedding_dim=128, use_faiss=True)

                    # Split data for evaluation
                    train_df, test_df = train_test_split(
                        self.df, test_size=test_size, random_state=42, stratify=self.df[self.target_col]
                    )
                    ...
                    test_preds_df = classifier.predict_batch(test_df[text_col])  # type: ignore[index]
                    y_true = test_df[self.target_col].tolist()  # type: ignore[index]
                    ...
                    try:
                        ...
                        classifier.generate_report(y_true, y_pred, report_path=report_path, embeddings=test_embeddings)
        ...
        if task_type == "image_clustering" and image_col:
            ...
            try:
                ...
                self.results = {
                    ...
                    "clusters": clusters.tolist(),  # type: ignore[union-attr]
        ...
        if task_type == "regression" and models_to_test:
            ...
            if use_ensemble and len(optimizer.model_results) > 1:
                ...
                logger.info(
                    f"✅ Ensemble CV Score: {ensemble_scores['mean_score']:.4f} (±{ensemble_scores['std_score']:.4f})"
                )

                self.model = ensemble.ensemble_model
                self.results = {"optimizer_results": optimizer.model_results, "ensemble_scores": ensemble_scores}
            else:
        ...
        if task_type == "classification" and models_to_test:
            ...
            optimizer = ClassifierOptimizer(self.df, self.target_col, self.feature_cols, models_to_test=models_to_test)
        ...
        y_pred = self.model.predict(X_test)  # type: ignore[union-attr]
        y_proba = None
        if task_type == "classification" and hasattr(self.model, "predict_proba"):
            y_proba = self.model.predict_proba(X_test)  # type: ignore[union-attr]

        validator = Validator(self.model)
        metrics = validator.evaluate(y_test, y_pred, task_type=task_type, y_proba=y_proba)  # type: ignore[arg-type]
    ...
    def predict(self, X):
        ...
        if hasattr(self.model, "predict_batch") and isinstance(X, (list, pd.Series)):
            if isinstance(X, list) and len(X) > 0 and isinstance(X[0], str):
                results_df = self.model.predict_batch(pd.Series(X))  # type: ignore[union-attr]
