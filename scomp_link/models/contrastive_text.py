...
try:
    from .contrastive_net import (
        ContrastiveDataset,
        ContrastiveLoss,
        ContrastiveSiameseModel,
        EarlyStopping,
        InfoNCELoss,
        SiameseDataset,
    )
    ...
    try:
        from scomp_link.models.contrastive_net import (
            ContrastiveDataset,
            ContrastiveLoss,
            ContrastiveSiameseModel,
            EarlyStopping,
...
class EmbeddingSelector:
        ...
        for model_name in self.candidates:
            ...
            try:
                if model_name in precomputed_embeddings:
                    ...
                    emb_result = st_model.encode(texts, show_progress_bar=False, normalize_embeddings=True)
                    encode_time = time.perf_counter() - t0
                    embeddings = np.asarray(emb_result)

                # Compute contrastive loss on pairs from the sample
                loss = self._compute_sample_loss(np.asarray(embeddings), labels)
    ...
    def _compute_sample_loss(self, embeddings: np.ndarray, labels: list, n_pairs: int = 2000) -> float:
        """
...
class ContrastiveTextClassifier:
    ...
    def __init__(self, model_name="bert-base-uncased", use_faiss=True, embedding_dim=256):
        ...
        self.model.train()
        self.siamese_model = ContrastiveSiameseModel(self.model, embedding_dim=embedding_dim)  # type: ignore[misc]
    ...
    def train_contrastive(
        ...
        if use_weighted_sampling:
            sample_weights = ContrastiveDataset.get_sample_weights(train_df, label_col=label_col)  # type: ignore[union-attr]
        ...
        # Loss function
        if loss_fn == "infonce":
            criterion = InfoNCELoss(temperature=0.07)  # type: ignore[misc]
            logger.info("✅ Using InfoNCE loss (temperature=0.07)")
        else:
            criterion = ContrastiveLoss(margin=1.0)  # type: ignore[misc]
        ...
        early_stopping = EarlyStopping(patience=early_stopping_patience, min_delta=0.001)  # type: ignore[misc]
        ...
        with torch.no_grad():
            for label in tqdm(unique_labels, desc="Encoding labels"):
                tokenized = self.tokenizer(
                    str(label), return_tensors="pt", truncation=True, padding="max_length", max_length=128
                )
                emb = self.siamese_model.forward_one(tokenized["input_ids"], tokenized["attention_mask"]).cpu().numpy()
    ...
    def embed(self, texts: Union[List[str], pd.Series], batch_size: int = 512) -> np.ndarray:
        ...
        with torch.no_grad():
            for i in range(0, len(texts), batch_size):
                ...
                tokenized = self.tokenizer(
                    batch_texts, return_tensors="pt", truncation=True, padding="max_length", max_length=128
                )
                batch_emb = (
                    self.siamese_model.forward_one(tokenized["input_ids"], tokenized["attention_mask"]).cpu().numpy()
                )
    ...
    def fit_head(
        ...
        y_arr = np.asarray(y)  # ensure ndarray for type checker
            ...
            for name, model in candidates:
                try:
                    scores = cross_val_score(
                        model, X_emb, y_arr, cv=min(5, len(np.unique(y_arr))), scoring="accuracy", n_jobs=-1
                    )
        ...
        return {
            ...
            "n_classes": int(len(le.classes_)),  # type: ignore[arg-type]
        }
    ...
    def _get_head_candidates(self, head: str) -> List[tuple]:
        ...
        if head in ("auto", "logreg"):
            candidates.append(("LogisticRegression", LogisticRegression(max_iter=1000, C=1.0, random_state=42)))
        if head in ("auto", "svm"):
            candidates.append(("LinearSVC", LinearSVC(max_iter=2000, C=1.0, random_state=42)))
        if head in ("auto", "rf"):
            candidates.append(("RandomForest", RandomForestClassifier(n_estimators=100, random_state=42, n_jobs=-1)))
        ...
        if head in ("auto", "lgbm"):
            try:
                ...
                candidates.append(
                    (
                        "LightGBM",
                        LGBMClassifier(n_estimators=200, learning_rate=0.1, random_state=42, verbosity=-1, n_jobs=-1),
                    )
                )
            except ImportError:
                if head == "lgbm":
                    logger.info("⚠️ LightGBM not installed, falling back to LogReg")
                    candidates.append(("LogisticRegression", LogisticRegression(max_iter=1000, random_state=42)))
        ...
        if head in ("auto", "xgb"):
            try:
                ...
                candidates.append(
                    (
                        "XGBoost",
                        XGBClassifier(n_estimators=200, learning_rate=0.1, random_state=42, verbosity=0, n_jobs=-1),
                    )
                )
            except ImportError:
                if head == "xgb":
    ...
    def predict(self, text: str, top_k: int = 1, return_confidence: bool = False):
        ...
        # Encode text
        tokenized = self.tokenizer(text, return_tensors="pt", truncation=True, padding="max_length", max_length=128)
        with torch.no_grad():
            text_emb = (
                self.siamese_model.forward_one(tokenized["input_ids"], tokenized["attention_mask"])
                .cpu()
                .numpy()
                .reshape(1, -1)
                .astype("float32")
        ...
        if self._head_model is not None and top_k == 1 and not return_confidence:
            assert self._head_label_encoder is not None
            pred_idx = self._head_model.predict(text_emb)[0]
            result = self._head_label_encoder.inverse_transform([pred_idx])
        ...
        if self._head_model is not None and hasattr(self._head_model, "predict_proba"):
            ...
            inv_result = self._head_label_encoder.inverse_transform(top_indices)
            predictions = list(inv_result)  # type: ignore[arg-type]
        ...
        if self._head_model is not None:
            ...
            if hasattr(self._head_model, "predict_proba"):
                ...
                for i in range(len(texts)):
                    inv_result = self._head_label_encoder.inverse_transform(pred_indices[i])
                    preds = list(inv_result)  # type: ignore[arg-type]
                ...
                pred_labels = self._head_label_encoder.inverse_transform(preds)  # type: ignore[arg-type]
                return pd.DataFrame(
                    {
                        "text": texts,
    ...
    def generate_report(
        ...
        y_true,
        y_pred,
        report_path: str = "contrastive_report.html",
        embeddings: Optional[np.ndarray] = None,
        selector_results: Optional[pd.DataFrame] = None,
    ) -> str:
        """
        ...
        fig_f1 = go.Figure(go.Bar(x=cls_df.index.tolist(), y=cls_df["f1-score"].tolist(), marker_color="#6E37FA"))
        fig_f1.update_layout(title="F1-Score per Class", xaxis_tickangle=-45, yaxis_title="F1", height=400)
        ...
        if selector_results is not None:
            ...
            fig_sel = go.Figure(
                go.Bar(
                    x=selector_results["model"].tolist(), y=selector_results["loss"].tolist(), marker_color="#34d399"
                )
            )
            fig_sel.update_layout(
                title="Contrastive Loss by Backbone (lower = better)",
                xaxis_tickangle=-45,
                yaxis_title="Loss",
                height=350,
            )
        ...
        if embeddings is not None and len(embeddings) <= 5000:
            ...
            try:
                ...
                fig_tsne = px.scatter(
                    viz_df, x="x", y="y", color="label", title="t-SNE of Contrastive Embeddings", height=500
                )
    ...
    def load(self, path: str = "./ContrastiveTextModel", model_name: Optional[str] = None):
        ...
        # Load weights
        self.siamese_model.load_state_dict(torch.load(os.path.join(path, "model.pt"), map_location=torch.device("cpu")))
        self.siamese_model.eval()
