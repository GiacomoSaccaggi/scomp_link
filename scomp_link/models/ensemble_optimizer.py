    ...
    def create_ensemble(self, meta_model=None):
        ...
        if self.strategy == "voting":
            if self.task_type == "classification":
                self.ensemble_model = VotingClassifier(estimators=self.base_models, voting="soft")
            else:
                self.ensemble_model = VotingRegressor(estimators=self.base_models)

        elif self.strategy == "stacking":
            if self.task_type == "classification":
                self.ensemble_model = StackingClassifier(estimators=self.base_models, final_estimator=meta_model, cv=5)
            else:
                self.ensemble_model = StackingRegressor(estimators=self.base_models, final_estimator=meta_model, cv=5)
