...
class SpacyEmbeddingModel(object):
    ...
    def __init__(self, lan: str = "en", model_name: str = "bert-base-uncased"):
        ...
        if not result or len(result) != 2:
            raise ValueError(
                f"Failed to load spaCy model for language '{lan}'. "
    ...
    def _select_language(self, lan: str, dict_size: str = "sm"):
        """
        Internal function for selecting languages.

        :param lan: Language in ISO 639-1
        :param dict_size: size of dictionary ('sm', 'md', 'lg', 'trf')
        ...
        if lan in self.all_spacy_lan:
            ...
            logger.info(
                f'\033[91m\tLanguage not managed, select in this languages:\n\t{",".join(self.all_spacy_lan)}\n\t'
    ...
    def __extract_words(self, sentence: str):
        ...
        words = [
            ...
            if (word.pos_ == "VERB" or word.pos_ == "AUX" or word.pos_ == "NOUN" or word.pos_ == "PROPN")
    ...
    def cnn_embedding_textcategorizer(
        self,
        texts,
        y_lab,
        texts_test,
        ...
        assert self.nlp.lang is not None
        ...
        logger.info("Number of examples ({} training, {} evaluation)".format(len(train_texts), len(dev_texts)))
        ...
        if True:  # training block
            ...
            for i in range(epoch):
                ...
                if use_tqdm:
                    try:
                        batches_iter = tqdm.tqdm(list(minibatch(train_examples, size=batch_size)), leave=False)
                    except Exception:
        ...
        for val in dev_texts:
            doc = textcat_nlp(val)
            fitted_y_test.append(max(doc.cats, key=lambda k: doc.cats[k]))
        fitted_y_train = []
        for val in train_texts:
            doc = textcat_nlp(val)
            fitted_y_train.append(max(doc.cats, key=lambda k: doc.cats[k]))
        ...
        previsioni_scores = {
            "classification_report_tr": clasification_report_tr,
