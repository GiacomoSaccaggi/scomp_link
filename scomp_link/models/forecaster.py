...
class TimeSeriesForecaster:
    ...
    def fit(self, series: Union[pd.Series, np.ndarray], **kwargs) -> "TimeSeriesForecaster":
        ...
        if method == "arima":
            ...
            logger.info(f"  ✅ ARIMA{order} fitted (AIC={self._fitted.aic:.2f})")  # type: ignore[union-attr]
            ...
            logger.info(f"  ✅ SARIMA{order}x{seasonal_order} fitted (AIC={self._fitted.aic:.2f})")  # type: ignore[union-attr]
            ...
            self._model = ExponentialSmoothing(
                series, trend="add", seasonal=seasonal, seasonal_periods=s if seasonal else None
            )
    ...
    def predict(self, steps: Optional[int] = None) -> pd.Series:
        ...
        forecast = self._fitted.forecast(h)  # type: ignore[union-attr]
    ...
    def predict_with_ci(self, steps: Optional[int] = None, alpha: float = 0.05) -> pd.DataFrame:
        ...
        if self._fitted is None:
            raise ValueError("Call fit() first.")
        h = steps or self.horizon
        pred = self._fitted.get_forecast(h)  # type: ignore[union-attr]
    ...
    @timer
    def walk_forward_cv(
        self, series: Union[pd.Series, np.ndarray], n_splits: int = 5, horizon: Optional[int] = None
    ) -> Dict:
        """
    ...
    def plot_forecast(self, forecast: Optional[pd.Series] = None, ci: Optional[pd.DataFrame] = None) -> "go.Figure":
        ...
        fig.add_trace(
            go.Scatter(
                x=x_forecast, y=forecast.values, name="Forecast", mode="lines", line=dict(dash="dash", color="red")
            )
        )
