...
class Validator:
    ...
    def evaluate(
        self, y_true, y_pred, task_type: str = "regression", y_proba: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        """
    ...
    def generate_validation_report(
        ...
        if task_type == "regression":
            ...
            fig1.add_trace(
                go.Scatter(
                    x=y_true, y=y_pred, mode="markers", name="Predictions", marker=dict(color="#6E37FA", opacity=0.5)
                )
            )
            fig1.add_trace(
                go.Scatter(
            ...
            fig1 = go.Figure(
                data=go.Heatmap(
                    z=cm, x=labels, y=labels, colorscale="Viridis", text=cm, texttemplate="%{text}", hoverinfo="z"
                )
            )
            ...
            if y_proba is not None:
                ...
                fig2.update_layout(
                    title="Probability Distributions", barmode="overlay", xaxis_title="Confidence", yaxis_title="Count"
                )
