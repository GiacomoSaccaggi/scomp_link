...
def _compute_psi_worker(args: tuple) -> Dict:
    ...
    # KS test
    ks_result = stats.ks_2samp(ref_vals, curr_vals)
    ks_stat: float = float(getattr(ks_result, "statistic", ks_result[0]))  # type: ignore[arg-type]
    p_value: float = float(getattr(ks_result, "pvalue", ks_result[1]))  # type: ignore[arg-type]
...
class DriftDetector:
    ...
    def _compute_ks(self, ref_col: np.ndarray, curr_col: np.ndarray) -> Dict:
        ...
        result = stats.ks_2samp(ref_col, curr_col)
        stat: float = float(getattr(result, "statistic", result[0]))  # type: ignore[arg-type]
        p_value: float = float(getattr(result, "pvalue", result[1]))  # type: ignore[arg-type]
        return {"ks_statistic": stat, "p_value": p_value, "drifted": p_value < self.ks_alpha}
    ...
    def detect(self, current: pd.DataFrame, features: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        for col in cols:
            ...
            if len(ref_vals) < 2 or len(curr_vals) < 2:
                continue
            work_items.append((col, ref_vals, curr_vals, self.psi_bins, self.ks_alpha, self.psi_threshold))
    ...
    def plot_feature_distribution(self, feature: str, current: pd.DataFrame):
        ...
        fig = go.Figure()
        fig.add_trace(go.Histogram(x=self.reference[feature].dropna(), name="Reference", opacity=0.6, nbinsx=40))
        fig.add_trace(go.Histogram(x=current[feature].dropna(), name="Current", opacity=0.6, nbinsx=40))
        fig.update_layout(barmode="overlay", title=f"Distribution: {feature}", xaxis_title=feature, yaxis_title="Count")
        return fig
