...
class ShapExplainer:
    ...
    def plot_importance(self, top_n: int = 20):
        ...
        fig = px.bar(
            ...
            labels={"mean_abs_shap": "Mean |SHAP|"},
        )
...
class LimeExplainer:
    ...
    def __init__(
        self, model, X_train: pd.DataFrame, task: str = "regression", feature_names: Optional[List[str]] = None
    ):
    ...
    def feature_importance(self, X: pd.DataFrame, n_samples: int = 50, num_features: int = 10) -> pd.DataFrame:
        ...
        df = pd.DataFrame([{"feature": k, "mean_abs_weight": np.mean(v)} for k, v in importance_acc.items()])
...
if __name__ == "__main__":
    ...
    assert isinstance(X_train_sample, pd.DataFrame)
    assert isinstance(X_test_sample, pd.DataFrame)
    shap_exp = ShapExplainer(model, X_train_sample)
    shap_exp.explain(X_test_sample)
