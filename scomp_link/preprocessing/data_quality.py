...
class DataQualityReport:
    ...
    def _missing_analysis(self) -> pd.DataFrame:
        ...
        df = pd.DataFrame({"column": missing.index, "missing_count": missing.values, "missing_pct": pct.values})
        df = df[df["missing_count"] > 0].sort_values("missing_pct", ascending=False)  # type: ignore[call-overload]
    ...
    def _type_inference(self) -> pd.DataFrame:
        ...
        for col in self.df.columns:
            ...
            inferred = "numeric" if pd.api.types.is_numeric_dtype(self.df[col]) else dtype
            if dtype == "object":
        ...
        for i in range(len(corr.columns)):
            for j in range(i + 1, len(corr.columns)):
                if corr.iloc[i, j] >= threshold:
                    pairs.append(
                        {"col_a": corr.columns[i], "col_b": corr.columns[j], "correlation": round(corr.iloc[i, j], 4)}
                    )
    ...
    def save_html(self, path: str = "data_quality_report.html"):
        ...
        if self._report is None:
            self.generate()
        assert self._report is not None  # for type checker
        r = self._report
