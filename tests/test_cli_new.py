...
def regression_csv(tmp_dir):
    np.random.seed(42)
    df = pd.DataFrame({"x1": np.random.randn(100), "x2": np.random.randn(100), "y": 2 * np.random.randn(100) + 0.5})
...
class TestEngineerCommand:
    def test_engineer_interactions(self, regression_csv, tmp_dir):
        ...
        code = run_cli(
            "engineer", "--data", regression_csv, "--target", "y", "--interactions", "--output", output, "--silent"
        )
        assert code == 0
    ...
    def test_engineer_log_transform(self, tmp_dir):
        ...
        df.to_csv(path, index=False)
        output = str(tmp_dir / "eng.csv")
        code = run_cli("engineer", "--data", path, "--target", "y", "--log-transform", "--output", output, "--silent")
        assert code == 0
    ...
    def test_engineer_no_target(self, regression_csv, tmp_dir):
        output = str(tmp_dir / "eng.csv")
        code = run_cli("engineer", "--data", regression_csv, "--interactions", "--output", output, "--silent")
        assert code == 0
...
class TestForecastCommand:
    ...
    def test_forecast_missing_column(self, series_csv):
        code = run_cli("forecast", "--data", series_csv, "--column", "nonexistent", "--horizon", "5", "--silent")
        assert code != 0
...
class TestAnomalyCommand:
    def test_anomaly_iforest_lof(self, regression_csv, tmp_dir):
        ...
        code = run_cli("anomaly", "--data", regression_csv, "--methods", "iforest,lof", "--output", output, "--silent")
        assert code == 0
        df = pd.read_csv(output)
...
class TestReportCommand:
    ...
    def test_report_model(self, regression_csv, tmp_dir):
        ...
        ScompArtifact().set_model(model).set_config(task_type="regression", target_col="y").set_metrics(
            {"r2": 0.5}
        ).set_feature_schema(df[["x1", "x2"]]).save(artifact_path)

        output = str(tmp_dir / "model_report.html")
        code = run_cli("report", "--artifact", artifact_path, "--data", regression_csv, "--output", output, "--silent")
        assert code == 0
        assert os.path.exists(output)
        with open(output) as f:
            html = f.read()
    ...
    def test_report_model_without_data(self, tmp_dir):
        ...
        ScompArtifact().set_model(model).set_config(task_type="regression").set_metrics({"r2": 0.99}).save(
            artifact_path
        )
...
class TestNewCLICommands:
    ...
    def regression_csv(self, tmp_dir):
        np.random.seed(42)
        n = 150
        df = pd.DataFrame({"x1": np.random.randn(n), "x2": np.random.randn(n), "y": 2 * np.random.randn(n) + 0.5})
    ...
    def classification_csv(self, tmp_dir):
        np.random.seed(42)
        n = 100
        df = pd.DataFrame({"x1": np.random.randn(n), "x2": np.random.randn(n), "y": np.random.choice([0, 1], n)})
    ...
    def test_cluster_kmeans(self, regression_csv, tmp_dir):
        ...
        args = parser.parse_args(
            ["cluster", "--data", regression_csv, "--n-clusters", "3", "--output", out, "--silent"]
        )
        args.func(args)
        assert Path(out).exists()
    ...
    def test_cluster_with_plot(self, regression_csv, tmp_dir):
        ...
        args = parser.parse_args(["cluster", "--data", regression_csv, "--n-clusters", "2", "--plot", plot, "--silent"])
        args.func(args)
    ...
    def test_text_tfidf(self, text_csv):
        ...
        parser = build_parser()
        args = parser.parse_args(
            ["text", "--data", text_csv, "--text-col", "msg", "--target", "cat", "--method", "tfidf", "--silent"]
        )
        args.func(args)
    ...
    def test_monitor(self, tmp_dir):
        ...
        args = parser.parse_args(
            ["monitor", "--reference", ref_path, "--current", cur_path, "--output", out, "--silent"]
        )
        args.func(args)
        assert Path(out).exists()
    ...
    def test_run_format_table(self, regression_csv):
        ...
        parser = build_parser()
        args = parser.parse_args(
            ["run", "--data", regression_csv, "--target", "y", "--task", "regression", "--format", "table", "--silent"]
        )
        args.func(args)
...
class TestTextNewArgs:
    ...
    def test_text_parser_select_backbone_flag(self, text_csv):
        ...
        parser = build_parser()
        args = parser.parse_args(
            ["text", "--data", text_csv, "--text-col", "msg", "--target", "cat", "--select-backbone", "--silent"]
        )
...
class TestEmbedCommand:
    ...
    def test_embed_parser_exists(self):
        ...
        parser = build_parser()
        args = parser.parse_args(["embed", "--data", "test.csv", "--text-col", "text", "--artifact", "model.scomp"])
