...
from sklearn.linear_model import ElasticNet, Lasso, SGDClassifier, SGDRegressor
...
def large_classification_df():
    ...
    for cat, (ma, mi) in zip(["Low", "Medium", "High"], [(30, 30000), (45, 60000), (55, 100000)]):
        for _ in range(n_per):
            rows.append([np.random.normal(ma, 5), np.random.normal(mi, 5000), np.random.choice(["A", "B", "C"]), cat])
...
class TestNumericalSmall:
    def test_econometric_model_selected(self, small_regression_df):
        ...
        pipe.choose_model(
            "numerical_prediction", metadata={"only_numerical_exogenous": True, "all_variables_important": True}
        )
...
class TestNumericalMediumLasso:
    def test_lasso_elasticnet_pipeline(self, medium_regression_df):
        ...
        pipe.choose_model(
            "numerical_prediction", metadata={"only_numerical_exogenous": True, "all_variables_important": False}
        )
        models_to_test = {
...
class TestNumericalMixedFeatures:
    def test_mixed_features_pipeline(self, mixed_features_df):
        ...
        pipe.choose_model(
            "numerical_prediction", metadata={"only_numerical_exogenous": False, "all_variables_important": True}
        )
        models_to_test = {
            "RandomForest": {
...
class TestClassificationSmall:
    def test_svc_knn_nb_pipeline(self, small_classification_df):
        ...
        pipe.choose_model("categorical_known", metadata={"records_per_category": 100, "exogenous_type": "mixed"})
        models_to_test = {
...
class TestClassificationLarge:
    def test_large_classification_pipeline(self, large_classification_df):
        ...
        pipe.choose_model("categorical_known", metadata={"records_per_category": 400, "exogenous_type": "mixed"})
        models_to_test = {
            "SGD": {
...
class TestAnomalyDetector:
    ...
    def test_report_no_groupby(self, anomaly_df):
        ...
        detector = AnomalyDetector(contamination=0.05, methods=["iforest", "lof"], consensus_threshold=1, verbose=False)
