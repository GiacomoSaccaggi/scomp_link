...
class TestReportProgress:
    def test_report_progress_runs(self):
        ...
        model.report_progress(
            epoch=1, best=0.9, losses={"textcat": 0.5}, scores={"textcat_p": 0.8, "textcat_r": 0.7, "textcat_f": 0.75}
        )
...
class TestSelectLanguage:
    ...
    def test_supported_language_loads(self):
        ...
        with patch("spacy.load", return_value=mock_nlp), patch("importlib.import_module", return_value=mock_module):
...
class TestSelectLanguageEdgeCases:
    def test_spacy_load_fallback_chain(self):
        ...
        with patch("spacy.load", side_effect=side_effect), patch("importlib.import_module", return_value=mock_module):
            result = model._select_language("en", "md")
...
class TestCNNEmbeddingTextcategorizer:
    ...
    def real_model(self):
        ...
        with patch.object(AT, "from_pretrained") as mock_tok, patch.object(AM, "from_pretrained") as mock_bert:
