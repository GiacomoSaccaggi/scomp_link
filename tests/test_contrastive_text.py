...
class TestEmbeddingSelector:
    def test_offline_mode(self, sample_df):
        ...
        results = selector.find_best_backbone(
            sample_df, text_col="text", label_col="label", precomputed_embeddings=precomputed
        )
...
class TestEmbed:
    def test_embed_returns_correct_shape(self, mock_classifier):
        ...
        mock_classifier.siamese_model.forward_one = MagicMock(return_value=torch.randn(3, 64))
    ...
    def test_embed_single_text(self, mock_classifier):
        ...
        mock_classifier.siamese_model.eval = MagicMock()
        mock_classifier.siamese_model.forward_one = MagicMock(return_value=torch.randn(1, 64))
...
class TestFitHead:
    def test_fit_head_logreg(self, mock_classifier, sample_df):
        ...
        np.random.seed(42)
        mock_classifier.embed = MagicMock(return_value=np.random.randn(len(sample_df), 64).astype("float32"))
...
class TestGenerateReport:
    ...
    def test_report_with_embeddings(self, mock_classifier, tmp_path):
        ...
        result = mock_classifier.generate_report(y_true, y_pred, report_path=report_path, embeddings=embeddings)
        assert os.path.exists(result)
    ...
    def test_report_with_selector_results(self, mock_classifier, tmp_path):
        ...
        result = mock_classifier.generate_report(
            ["a", "b", "a"], ["a", "b", "b"], report_path=report_path, selector_results=selector_df
        )
        assert os.path.exists(result)
