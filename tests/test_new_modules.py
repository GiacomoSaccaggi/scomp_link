...
class TestShapExplainer:
    ...
    def test_feature_importance_ndim_gt1_multiclass(self):
        ...
        X, y = make_classification(n_samples=120, n_features=6, n_classes=3, n_informative=4, random_state=42)
...
class TestLimeExplainer:
    ...
    def test_feature_names_explicit(self, regression_model_and_data):
        ...
        model, X_train, X_test, _, _ = regression_model_and_data
        explainer = LimeExplainer(model, X_train, task="regression", feature_names=["a", "b", "c"])
...
class TestOptunaOptimizer:
    def test_optimize_returns_fitted_model(self):
        ...
        optimizer = OptunaOptimizer(GradientBoostingRegressor, param_space, scoring="r2", n_trials=5, cv=3)
...
class TestHalvingSearchOptimizer:
    def test_optimize_returns_best_model(self):
        ...
        optimizer = HalvingSearchOptimizer(
            GradientBoostingRegressor(), {"n_estimators": [10, 30, 50], "max_depth": [2, 3, 5]}, scoring="r2", cv=3
        )
        best_model = optimizer.optimize(X, y, verbose=False)
        assert hasattr(best_model, "predict")
    ...
    def test_results_dataframe(self):
        ...
        optimizer = HalvingSearchOptimizer(
            GradientBoostingRegressor(), {"n_estimators": [10, 30], "max_depth": [2, 3]}, scoring="r2", cv=2
        )
        optimizer.optimize(X, y, verbose=False)
...
class TestEarlyStoppingCV:
    def test_finds_optimal_iterations(self):
        ...
        stopper = EarlyStoppingCV(
            GradientBoostingRegressor(learning_rate=0.1), max_iterations=100, patience=30, step=10, cv=3
        )
...
class TestDriftDetector:
    ...
    def test_detect_with_drift(self):
        ...
        detector = DriftDetector(ref)
        report = detector.detect(curr)
        assert report[report["feature"] == "x"]["drifted"].values[0]
        assert not report[report["feature"] == "y"]["drifted"].values[0]
    ...
    def test_psi_threshold_respected(self):
        ...
        assert report.iloc[0]["psi_drifted"]
...
class TestScompArtifact:
    ...
    def test_is_scomp_file(self, regression_model_and_data):
        ...
        try:
            ScompArtifact().set_model(model).save(path)
            assert ScompArtifact.is_scomp_file(path)
            assert not ScompArtifact.is_scomp_file("/nonexistent/file.scomp")
        finally:
            os.unlink(path)
