...
from scomp_link.cli import build_parser, main
...
def classification_csv(tmp_dir):
    np.random.seed(42)
    df = pd.DataFrame({"x1": np.random.randn(100), "x2": np.random.randn(100), "y": np.random.choice([0, 1], 100)})
...
class TestRunCommand:
    def test_run_regression(self, regression_csv, tmp_dir):
        ...
        code = run_cli(
            "run", "--data", regression_csv, "--target", "y", "--task", "regression", "--output", output, "--silent"
        )
        assert code == 0
    ...
    def test_run_parquet_input(self, parquet_file, tmp_dir):
        output = str(tmp_dir / "results.json")
        code = run_cli(
            "run", "--data", parquet_file, "--target", "target", "--task", "regression", "--output", output, "--silent"
        )
        assert code == 0
    ...
    def test_run_missing_file(self):
        code = run_cli("run", "--data", "/nonexistent.csv", "--target", "y", "--task", "regression", "--silent")
        assert code != 0

    def test_run_missing_target(self, regression_csv):
        code = run_cli("run", "--data", regression_csv, "--target", "nonexistent", "--task", "regression", "--silent")
        assert code != 0

    def test_run_stdout_output(self, regression_csv, capsys):
        code = run_cli("run", "--data", regression_csv, "--target", "y", "--task", "regression", "--silent")
        assert code == 0
        captured = capsys.readouterr()
...
class TestPredictCommand:
    def test_predict_csv_output(self, artifact_path, regression_csv, tmp_dir):
        ...
        code = run_cli("predict", "--artifact", artifact_path, "--data", regression_csv, "--output", output, "--silent")
        assert code == 0
    ...
    def test_predict_invalid_artifact(self, regression_csv):
        code = run_cli("predict", "--artifact", "/nonexistent.scomp", "--data", regression_csv, "--silent")
        assert code != 0
...
class TestDriftCommand:
    def test_drift_no_drift(self, regression_csv, tmp_dir):
        ...
        code = run_cli(
            "drift", "--reference", regression_csv, "--current", regression_csv, "--output", output, "--silent"
        )
        assert code == 0
        df = pd.read_csv(output)
    ...
    def test_drift_with_drift(self, regression_csv, tmp_dir):
        ...
        np.random.seed(99)
        pd.DataFrame({"x1": np.random.randn(100) + 5, "x2": np.random.randn(100), "y": np.random.randn(100)}).to_csv(
            drifted_path, index=False
        )

        output = str(tmp_dir / "drift.csv")
        code = run_cli(
            "drift", "--reference", regression_csv, "--current", drifted_path, "--output", output, "--silent"
        )
        assert code == 0
        df = pd.read_csv(output)
        assert df[df["feature"] == "x1"]["drifted"].values[0]
