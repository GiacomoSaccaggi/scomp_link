...
report.add_rawgraphs_to_report(
    barchart(["Q1", "Q2", "Q3", "Q4"], [120, 180, 150, 210], "Quarterly Revenue"), "Bar Chart"
)
report.add_rawgraphs_to_report(
    barchartmultiset(["Jan", "Feb", "Mar"], {"Product A": [30, 45, 28], "Product B": [22, 38, 41]}, "Sales"),
    "Multi-set Bar Chart",
)
report.add_rawgraphs_to_report(
    barchartstacked(["2021", "2022", "2023"], {"Online": [100, 150, 200], "Retail": [80, 90, 110]}, "Revenue"),
    "Stacked Bar Chart",
)
report.add_rawgraphs_to_report(piechart(["Desktop", "Mobile", "Tablet"], [55, 35, 10], "Device Share"), "Pie Chart")
report.add_rawgraphs_to_report(
    radarchart(
        ["Speed", "Power", "Range", "Durability", "Accuracy"],
        {"Player A": [8, 6, 7, 9, 5], "Player B": [6, 9, 5, 7, 8]},
        "Stats",
    ),
    "Radar Chart",
)
report.add_rawgraphs_to_report(voronoidiagram(np.random.rand(12, 2).tolist(), title="Voronoi"), "Voronoi Diagram")
...
report.add_rawgraphs_to_report(
    bumpchart({"Apple": [1, 2, 1], "Google": [2, 1, 2], "MS": [3, 3, 3]}, ["2021", "2022", "2023"], "Rank"),
    "Bump Chart",
)
tasks = [
    {"name": "Design", "start": 0, "end": 3, "group": "P1"},
...
x = list(range(30))
report.add_rawgraphs_to_report(
    horizongraph({"Temp": (np.sin(np.linspace(0, 4 * np.pi, 30)) * 8).tolist()}, x, "Sensor"), "Horizon Graph"
)
report.add_rawgraphs_to_report(
    linechart(
        {"Revenue": [10, 15, 13, 18, 22], "Costs": [8, 9, 11, 12, 14]}, ["Jan", "Feb", "Mar", "Apr", "May"], "Trend"
    ),
    "Line Chart",
)
report.add_rawgraphs_to_report(slopechart({"A": [45, 62], "B": [70, 55]}, ["2022", "2023"], "Change"), "Slope Chart")
report.add_rawgraphs_to_report(
    streamgraph(
        {"Rock": np.random.randint(5, 20, 15).tolist(), "Pop": np.random.randint(3, 15, 15).tolist()},
        list(range(15)),
        "Music",
    ),
    "Streamgraph",
...
report.add_rawgraphs_to_report(
    bubblechart(
        np.random.rand(15).tolist(), np.random.rand(15).tolist(), (np.random.rand(15) * 80).tolist(), title="Bubbles"
    ),
    "Bubble Chart",
)
report.add_rawgraphs_to_report(
    contour_plot(np.random.normal(0, 1, 150).tolist(), np.random.normal(0, 1, 150).tolist(), "KDE"), "Contour Plot"
)
...
report.add_rawgraphs_to_report(convex_hull(x, y, ["A"] * 25 + ["B"] * 25, "Clusters"), "Convex Hull")
report.add_rawgraphs_to_report(
    hexagonal_binning(np.random.normal(0, 2, 300).tolist(), np.random.normal(0, 2, 300).tolist(), "Density"),
    "Hex Binning",
)
report.add_rawgraphs_to_report(
    matrixplot(np.random.rand(5, 5).tolist(), ["A", "B", "C", "D", "E"], ["V", "W", "X", "Y", "Z"], "Heatmap"),
    "Matrix Plot",
)
data = {
    "Speed": np.random.rand(20).tolist(),
    "Power": np.random.rand(20).tolist(),
    "Range": np.random.rand(20).tolist(),
    "Type": ["X"] * 10 + ["Y"] * 10,
}
...
report.add_rawgraphs_to_report(
    chord_diagram([[0, 5, 3], [5, 0, 4], [3, 4, 0]], ["X", "Y", "Z"], "Flows"), "Chord Diagram"
)
nodes = [{"name": "Solar", "x": 0}, {"name": "Wind", "x": 0}, {"name": "Grid", "x": 1}, {"name": "Home", "x": 2}]
links = [
