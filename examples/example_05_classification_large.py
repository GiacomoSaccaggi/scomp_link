...
for cat in categories:
    for _ in range(n_per_class):
        if cat == "Low":
            data.append(
                [np.random.normal(30, 5), np.random.normal(30000, 5000), np.random.choice(["A", "B", "C"]), cat]
            )
        elif cat == "Medium":
            data.append(
                [np.random.normal(45, 5), np.random.normal(60000, 10000), np.random.choice(["B", "C", "D"]), cat]
            )
        else:
            data.append(
                [np.random.normal(55, 5), np.random.normal(100000, 15000), np.random.choice(["C", "D", "E"]), cat]
            )
...
pipe.choose_model("categorical_known", metadata={"records_per_category": 1000, "exogenous_type": "mixed"})
